local cfg_itemcollection = require "modules.itemcollection.config"
...
RegisterNetEvent(
    "itemcollection:pickup",
    function(netid, entityModel)
        local src = source
        ...
        if not cfg_itemcollection[model] then
            return
        end
...
exports(
    ...
    function(event, item, inventory, slot, data)
        if event == "usingItem" then
            ...
            if veh ~= 0 then
                TriggerClientEvent(
                    "ox_lib:notify",
                    inventory.id,
                    {description = "Este item não pode ser usado em um veículo.", position = "top", type = "error"}
                )
                return false
            end
            return
        ...
        if event == "usedItem" then
            ...
            local entity =
                CreateObject(
                type(item.prop) == "table" and item.prop[1] or item.prop,
                coords.x,
                coords.y,
                coords.z,
                true,
