local cfg = require "modules.backup.config"
...
local function extractDbConfig(source)
    ...
    if cfg.Debug then
        SendNotification(
            ...
            "debug",
            string.format(
                "ExtractDbConfig: Usuário: %s, Senha: %s, Servidor: %s, Banco: %s",
                user,
                password,
                host,
                database
...
RegisterNetEvent(
    "onResourceStart",
    function(resource)
        if GetCurrentResourceName() == resource and cfg.Active and cfg.BackupOnStart then
            backupDatabase(0)
        end
    end
)

if cfg.Active and cfg.BackupCommand then
    lib.addCommand(
        cfg.BackupCommand,
        {
...
CreateThread(
    function()
        while true do
            if cfg.Active and cfg["ExecuteOnTime"] then
                local now = os.date("*t", os.time)
                if now.min == cfg.ExecuteOnTime.min and now.hour == cfg.ExecuteOnTime.hour then
                    backupDatabase(0)
                end
            end
            Wait(50000)
