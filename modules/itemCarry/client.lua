...
AddStateBagChangeHandler(
    "carryItem",
    nil,
    function(bagName, key, propData, _unused, replicated)
        local ply = GetPlayerFromStateBagName(bagName)
        local plyPed = GetPlayerPed(ply)

        if ply == 0 or plyPed ~= cache.ped then
            return
        end
        ...
        while currentCarryObject do
            ...
            if
                not IsEntityPlayingAnim(cache.ped, propData.dictionary, propData.animation, 3) and
                    not LocalPlayer.state.isdead and
                    not LocalPlayer.state.isDead
             then ---@todo may need to add dead checks and other things here as well
                TaskPlayAnim(
                    cache.ped,
                    propData.dictionary,
                    propData.animation,
