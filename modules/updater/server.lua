local cfg = require("modules.updater.config")
...
local destination = ""
 --GetResourcePath('progressbar'):gsub("/progressbar", "")
print(destination)
...
local function updateResource(source, args)
    ...
    local url =
        string.format(string.format(fileDownloadUrl, gitUser, splittedUrl[2], args.repoInfo.tag_name, "file_list.txt"))
    if cfg.Debug then
        SendNotification(
            source,
            "debug",
        ...
        function(reponseCode, resultData, resultHeaders, errorData)
            if reponseCode ~= 200 or resultData == nil then
                SendNotification(
                    ...
                    "error",
                    string.format(
                        "[%s] Erro ao obter lista de arquivos: %s, %s",
                        args.resourceName,
                        reponseCode,
                        json.encode(
                            errorData,
            ...
            for k, v in pairs(fileList) do
                if v ~= nil and v ~= "" then
                    ...
                    PerformHttpRequest(
                        string.format(fileDownloadUrl, gitUser, splittedUrl[2], args.repoInfo.tag_name, v),
                        function(reponseCode, resultData, resultHeaders, errorData)
                            if reponseCode ~= 200 or resultData == nil then
                                SendNotification(
                                    source,
                            ...
                            if cfg.Debug then
                                SendNotification(
                                    source,
                                    "debug",
                                    string.format("[%s] Baixado: %s", args.resourceName, v)
                                )
                            end
    ...
    SendNotification(
        ...
        "success",
        string.format(
            "[%s] Atualização concluída, reinicie o servidor para aplicar as alterações!",
            args.resourceName,
            args.repoInfo.tag_name
        ),
        cfg.WebHook,
        cfg.KeepOnConsole
    )
...
local function checkForUpdates(source, args)
    ...
    local version =
        string.match(fxmanifest, 'version%s*"%d+%.%d+%.%d+"') or string.match(fxmanifest, "version%s*'%d+%.%d+%.%d+'")
    if version then
    ...
    PerformHttpRequest(
        string.format(relaseInfoUrl, gitUser, args.resource),
        function(reponseCode, resultData, resultHeaders, errorData)
            if reponseCode == 200 and resultData ~= nil then
                ...
                SendNotification(
                    ...
                    "error",
                    string.format(
                        "[%s] Erro ao obter informações do repositório: %s, %s",
                        args.resource,
                        reponseCode,
                        json.encode(
                            errorData,
    ...
    if ver > version then
        SendNotification(
            source,
            "info",
            string.format("[%s] Nova versão encontrada: %s", args.resource, repoInfo.tag_name),
            cfg.WebHook,
            cfg.KeepOnConsole
        )
    else
        if cfg.Debug then
            SendNotification(
                source,
                "debug",
                string.format("[%s] Nenhuma nova versão encontrada", args.resource),
                cfg.WebHook,
                cfg.KeepOnConsole
            )
        end
