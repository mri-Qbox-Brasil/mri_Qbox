...
local function changeRole(args)
    ...
    if input then
        if
            ConfirmationDialog(
                string.format("Mudar '%s' de '%s' para '%s'?", args.vipData.name, args.vipData.role, input[1])
            ) == "confirm"
         then
            addVip(
                {
...
function ListVip()
    ...
    local ctx = {
        ...
        menu = "menu_vip",
        title = "Vips",
        description = string.format("Online: %d/%d", vipPeople.onlineVip, vipPeople.onlineVip + vipPeople.offlineVip),
        options = {}
    }
