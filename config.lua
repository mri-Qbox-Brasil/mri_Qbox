...
-----------------------------------------------------------------------------------------
cfg.wheelbreak = {
    toggle = false,          -- true para ativar o sistema de quebra/soltura de roda em colisão
    mode = "impact",         -- "impact" = usa força do impacto (perda brusca de lataria) | "speed" = usa velocidade mínima de colisão
    impactDamage = 80.0,     -- (somente mode "impact") dano mínimo de lataria perdido de uma vez para quebrar roda (quanto menor, mais sensível)
    minImpactSpeed = 20.0,   -- (somente mode "impact") velocidade mínima em km/h para validar impacto quando o native de colisão falhar
    impactDeltaSpeed = 35.0, -- (somente mode "impact") perda mínima de velocidade (km/h) em um instante para considerar colisão forte
    cooldown = 5000,         -- tempo em ms entre uma quebra e outra no mesmo veículo (evita spam em batidas seguidas)
    speed = 85,              -- (somente mode "speed") velocidade mínima em km/h para quebrar roda ao colidir
}
